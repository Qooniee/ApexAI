"""LSTM model with fully connected layer for time-series classification."""
...
class LSTMwithFC(nn.Module):
    """LSTM model with fully connected output layer.

    This model implements a Long Short-Term Memory (LSTM) network followed by
    ...
    Args:
        ...
        dropout_ratio: Dropout probability between LSTM layers.
    ...
    def __init__(
        ...
        self.cell_0 = torch.zeros(self.num_layers, 1, self.hidden_layer_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Initialize hidden and cell states for this batch
        self.hidden_0 = torch.zeros(self.num_layers, batch_size, self.hidden_layer_size).to(
            self.DEVICE
        )
        self.cell_0 = torch.zeros(self.num_layers, batch_size, self.hidden_layer_size).to(
            self.DEVICE
        )

        # LSTM forward pass
        lstm_out, (_h_n, _c_n) = self.lstm(x, (self.hidden_0, self.cell_0))
...
def main() -> None:
    """Test the LSTM model with sample data and export to ONNX format."""
    import onnx
    ...
    traced_script_module.save("LSTM_test.pt")
    logger.info("Model saved to LSTM_test.pt")
    ...
    lstm_onnx_model = onnx.load("LSTM_test.onnx")
    options = SessionOptions()
    options.graph_optimization_level = GraphOptimizationLevel.ORT_ENABLE_ALL

    logger.info("ONNX model structure:")
    logger.info(onnx.helper.printable_graph(lstm_onnx_model.graph))
